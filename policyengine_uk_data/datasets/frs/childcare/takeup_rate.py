...
def run_simulation(params, seed=42):
    ...
    # Take-up flags
    sim.set_input("would_claim_tfc", 2025, np.random.random(benunit_count) < tfc)
    sim.set_input("would_claim_extended_childcare", 2025, np.random.random(benunit_count) < extended)
    sim.set_input("would_claim_targeted_childcare", 2025, np.random.random(benunit_count) < targeted)
    sim.set_input("would_claim_universal_childcare", 2025, np.random.random(benunit_count) < universal)

    # Entitlement hours (clipped normal distribution per person)
    hours = np.clip(np.random.normal(loc=mean_hours, scale=stderr_hours, size=person_count), 0, 30)
    ...
    caseload = {
        "tfc": df[(df["tax_free_childcare"] > 0) & (df["age"] < 12)]["would_claim_tfc"].sum() / 1e3,
        "extended": df[(df["extended_childcare_entitlement"] > 0) & (df["age"] < 4) & (df["age"] > 0.9)]["would_claim_extended_childcare"].sum() / 1e3,
        "universal": df[(df["universal_childcare_entitlement"] > 0) & (df["age"] < 5) & (df["age"] >= 3)]["would_claim_universal_childcare"].sum() / 1e3,
        "targeted": df[(df["targeted_childcare_entitlement"] > 0) & (df["age"] < 3) & (df["age"] >= 2)]["would_claim_targeted_childcare"].sum() / 1e3,
    }
